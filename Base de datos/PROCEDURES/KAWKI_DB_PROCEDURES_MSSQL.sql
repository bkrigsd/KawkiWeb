...
-- =====================================================
-- SP_REPORTE_VENTAS_Y_TENDENCIAS - MS SQL Server
-- Reporte de rendimiento de ventas y tendencias de productos
-- =====================================================
USE KAWKI_DB;
GO

IF OBJECT_ID('SP_REPORTE_VENTAS_Y_TENDENCIAS', 'P') IS NOT NULL
    DROP PROCEDURE SP_REPORTE_VENTAS_Y_TENDENCIAS;
GO

CREATE PROCEDURE SP_REPORTE_VENTAS_Y_TENDENCIAS
    @p_fecha_inicio DATETIME,
    @p_fecha_fin DATETIME
AS
BEGIN
    ...
    -- Productos más vendidos con cantidad y monto total
    SELECT 
        p.PRODUCTO_ID,
    ...
    INNER JOIN TALLAS t ON pv.TALLA_ID = t.TALLA_ID
    WHERE v.ES_VALIDA = 1
      AND v.FECHA_HORA_CREACION BETWEEN @p_fecha_inicio AND @p_fecha_fin
    GROUP BY p.PRODUCTO_ID, p.DESCRIPCION, c.NOMBRE, e.NOMBRE, col.NOMBRE, t.NUMERO
    ORDER BY CANTIDAD_VENDIDA DESC, MONTO_TOTAL DESC;
END
GO

-- =====================================================
-- SP_REPORTE_ESTADO_STOCK - MS SQL Server
-- Reporte de estado de stock y productos en riesgo
-- =====================================================
USE KAWKI_DB;
GO

IF OBJECT_ID('SP_REPORTE_ESTADO_STOCK', 'P') IS NOT NULL
    DROP PROCEDURE SP_REPORTE_ESTADO_STOCK;
GO

CREATE PROCEDURE SP_REPORTE_ESTADO_STOCK
    @p_fecha_inicio DATETIME,
    @p_fecha_fin DATETIME
AS
BEGIN
    ...
    LEFT JOIN MOVIMIENTOS_INVENTARIO mi ON pv.PROD_VARIANTE_ID = mi.PROD_VARIANTE_ID
        AND mi.FECHA_HORA_MOV BETWEEN @p_fecha_inicio AND @p_fecha_fin
